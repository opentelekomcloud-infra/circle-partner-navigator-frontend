...
const requestPagination = async (url) => {
    ...
    let totalCount
    do {
        let localurl = `${url}&pagination[page]=${i}`
...
async function fetchPartnerMediaUrls() {
    ...
    data.map(item => {
        ...
        let partnerMedia = item.attributes.overview_media.data.attributes.url
        // partnerMedia = partnerMedia.substring(1);
        if (!mediaFileUrls.includes(partnerMedia)) {
            mediaFileUrls.push(partnerMedia);
        }

        let teaserMedia = item.attributes.teaser_media.data.attributes.url
...
async function fetchFeatureMediaUrls(mediaFileUrls) {
    ...
    data.map(item => {
        // Check for imageUrl existance
        if (!item.attributes.media.data) {
            throw new Error(`Feature \"${item.attributes.headline}\" doesn't have any media! Exiting.`)
        }
        let media = item.attributes.media.data.attributes.url
...
async function fetchQuotesMediaUrls(mediaFileUrls) {
    ...
    data.map(item => {
        // Check for imageUrl existance
        let media = item.attributes.profile_picture.data.attributes.url
        if (!mediaFileUrls.includes(media)) {
            mediaFileUrls.push(media);
        }
    })
