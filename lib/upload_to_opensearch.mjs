...
if (!username || !password) {
  throw new Error("Missing OPENSEARCH_USER or OPENSEARCH_PASSWORD environment variables.");
}
...
function pickLocalized(obj, field, targetLocale) {
  ...
  let result;
  if (loc && loc[field] != null && loc[field] !== '') {
    result = loc[field];
  } else {
    result = obj[field];
  }
...
const enIndex = "cpn-partners-test";
if (!(await client.indices.exists({ index: enIndex })).body) {
  await client.indices.create({ index: enIndex });
}
await bulkUploadTransformed(enIndex, transformEntries(PartnersData));

// German-localized index
const deIndex = "cpn-partners-test-de";
if (!(await client.indices.exists({ index: deIndex })).body) {
  await client.indices.create({ index: deIndex });
}
const transformedDe = transformEntriesLocalized(PartnersData, 'de-DE');
await bulkUploadTransformed(deIndex, transformedDe);
