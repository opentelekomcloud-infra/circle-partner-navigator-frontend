...
/** Create index if missing (supports mappings/settings). */
async function createIndexIfMissing(indexName, client, settingsAndMappings = {}) {
  const exists = await client.indices.exists({ index: indexName });
  const doesExist = exists.body ?? exists;
  if (!doesExist) {
    await client.indices.create({ index: indexName, ...settingsAndMappings });
  }
}
...
/** Shared index settings/mappings. */
const commonIndexBody = {
  body: {
    settings: {
      number_of_shards: 1,
    ...
    mappings: {
      properties: {
        id: { type: 'keyword' },
        'attributes.locale': { type: 'keyword' },
        'attributes.partner_id': { type: 'keyword' },
        'attributes.createdAt': { type: 'date' },
        'attributes.updatedAt': { type: 'date' },
        'attributes.publishedAt': { type: 'date' },
        'attributes.overview_headline': { type: 'text' },
        'attributes.overview_description': { type: 'text' },
        'attributes.teaser_headline': { type: 'text' },
        'attributes.teaser_description': { type: 'text' },
        'attributes.overview_product_type': { type: 'keyword' },
        'attributes.overview_company_name': { type: 'keyword' },
        'attributes.overview_website': { type: 'keyword' },
        'attributes.features': { type: 'object', enabled: true },
        'attributes.tags': { type: 'object', enabled: true },
        'attributes.quotes': { type: 'object', enabled: true }
      }
    }
  }
};
...
/** Build EN index: delete, create, bulk, refresh. */
const enIndex = cpn_index.concat('-en');
await deleteIndexIfExists(enIndex, client);
await createIndexIfMissing(enIndex, client, commonIndexBody);
...
/** Build DE index: delete, create, bulk, refresh. */
const deIndex = cpn_index.concat('-de');
await deleteIndexIfExists(deIndex, client);
await createIndexIfMissing(deIndex, client, commonIndexBody);
