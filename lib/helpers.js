...
const emailGatewayUrl = 'http://localhost:3003/api/v1/sendmail'
...
export default async function sendEmail(messageContent) {
    try {
        if (!messageContent["cap-token"]) {
        ...
        let requestBody = {
            ...
            captcha_token: messageContent["cap-token"]
        }
